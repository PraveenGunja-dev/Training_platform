...
interface ProfileState {
  subMentor?: SubMentorData;
  groupName?: string;
}

export default function SubMentorProfilePage() {
  const { state } = useLocation() as { state: ProfileState | null };
  const navigate = useNavigate();

  const ins = state?.subMentor;

  if (!ins) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        <GraduationCap className="h-12 w-12 text-slate-300" />
        <p className="text-sm text-slate-500">SubMentor info not available.</p>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-teal-100 text-teal-700 border border-teal-200">
                  <GraduationCap className="h-3 w-3" /> SubMentor
                </span>
                {state?.groupName && (
