/**
 * Chunk 6 — SubMentor write-flow tests
 *
 * Strategy:
...
vi.mock('@/features/sub-mentor/calendar/SubMentorCalendar', () => ({
  SubMentorCalendar: () => <div data-testid="subMentor-calendar" />,
}));
...
function makeSubMentor(): User {
  return {
    id: 'inst-1', email: 'subMentor@test.com', full_name: 'Test SubMentor',
...
describe('useCan — authorization matrix', () => {
  ...
  it('subMentor returns false for unassigned group', () => {
  ...
  it('subMentor returns true for assigned group (edit)', () => {
    useAuthStore.setState({ user: makeSubMentor(), accessToken: 'tok' });
    vi.mocked(useCan).mockReturnValue(true);
    const { result } = renderHook(() =>
  ...
  it('subMentor returns false for document delete when uploaded by someone else', () => {
    useAuthStore.setState({ user: makeSubMentor(), accessToken: 'tok' });
    vi.mocked(useCan).mockReturnValue(false);
    const { result } = renderHook(() =>
  ...
  it('subMentor returns true for document delete of own upload in assigned group', () => {
    useAuthStore.setState({ user: makeSubMentor(), accessToken: 'tok' });
    vi.mocked(useCan).mockReturnValue(true);
    const { result } = renderHook(() =>
...
describe('ClassDetailPage — write flows', () => {
  ...
  const clsOngoing: ClsData = {
    ...
    active_session: {
      ...
      started_by: { id: 'inst-1', full_name: 'Test SubMentor' },
  ...
  async function renderDetail() {
    ...
    render(wrapRouted(<ClassDetailPage />, '/sub-mentor/classes/cls-1', '/sub-mentor/classes/:id'));
  }
...
describe('AssignmentDetailPage — write flows', () => {
  ...
  async function renderDetail(task: typeof lockedTask) {
    ...
    render(wrapRouted(
      ...
      '/sub-mentor/assignments/t-1',
      '/sub-mentor/assignments/:id',
    ));
  }
