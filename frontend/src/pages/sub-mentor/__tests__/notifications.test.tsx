/**
 * Chunk 09 — SubMentor notification UI tests
 *
 * Covers:
...
describe('NotificationDropdown', () => {
  ...
  it('navigates to /subMentor/notifications for SUB_MENTOR role', async () => {
    const { useQuery, useMutation } = await import('@tanstack/react-query');
    vi.mocked(useQuery).mockReturnValue(qResult({
      data: { data: [] },
      isLoading: false,
...
describe('SubMentorNotificationsPage — preferences', () => {
  it('renders in-app toggle as disabled (always-on)', async () => {
    ...
    const SubMentorNotificationsPage = (await import('../NotificationsPage')).default;
    wrapper(<SubMentorNotificationsPage />);
