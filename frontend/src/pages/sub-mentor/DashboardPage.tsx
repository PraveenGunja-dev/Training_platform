...
export default function SubMentorDashboardPage() {
  const { data: dashboardData, isLoading: isDashboardLoading } = useQuery({
    queryKey: ['dashboard', 'subMentor'],
  ...
  const { data: myGroupsData, isLoading: isGroupsLoading } = useQuery({
    queryKey: ['subMentor', 'my-groups'],
    queryFn: () => subMentorApi.myGroups(),
    staleTime: 60_000,
  });

  if (isDashboardLoading || isGroupsLoading) return <DashboardSkeleton />;

  const groups = myGroupsData?.data ?? [];
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <div className="mb-4 rounded-full bg-blue-100 p-4">
          <FolderKanban className="h-8 w-8 text-[#0052A5]" />
        </div>
        <h3 className="text-lg font-semibold text-[#00285A]">No groups assigned</h3>
        <p className="mt-1 text-sm text-[#5A7A9A] max-w-sm">
          You have not been assigned to any groups yet. Please contact an admin.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[#00285A] leading-tight">SubMentor Dashboard</h1>
          <p className="text-sm text-[#5A7A9A]">
            {`Showing data for your ${N} assigned group${N !== 1 ? 's' : ''}.`}
          </p>
          {myGroupsData?.effective_can_view_all && (
