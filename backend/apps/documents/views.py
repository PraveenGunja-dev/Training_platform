...
from apps.assignments.storage import generate_download_sas, generate_upload_sas
...
class DocumentViewSet(ViewSet):
    ...
    def _base_qs(self):
        return Document.objects.select_related("group", "class_obj", "uploaded_by")
    ...
    # DELETE /documents/:id (Admin or Instructor on assigned group)
    ...
    def download(self, request: Request, pk: str | None = None) -> Response:
        ...
        download_url = generate_download_sas(doc.file_url)
...
@extend_schema(exclude=True)
class ParticipantSharedUploadUrlView(APIView):
    """POST /groups/:group_id/shared-upload-url — presigned PUT URL for participant file upload."""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request: Request, group_id: str) -> Response:
        ...
        if not filename:
            return _validation_error({"filename": "This field is required."})
        blob_name = f"shared-docs/{uuid_lib.uuid4()}/{filename}"
...
class DocumentUploadUrlView(APIView):
    """POST /documents/upload-url — return a presigned PUT URL for direct file upload."""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request: Request) -> Response:
        ...
        if not filename:
            return _validation_error({"filename": "This field is required."})
        blob_name = f"documents/{uuid_lib.uuid4()}/{filename}"
        upload_url = generate_upload_sas(blob_name, content_type)
        return Response({"data": {"upload_url": upload_url, "blob_name": blob_name}})
